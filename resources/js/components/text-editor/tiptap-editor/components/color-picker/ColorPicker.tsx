...
const ColorPicker = (props: ColorPickerProps) => {
   ...
   const isColorEqual = (a: string, b: string): boolean => normalizeColor(a).toUpperCase() === normalizeColor(b).toUpperCase();
   ...
   const renderColorList = (colors: string[], label: string) => (
      <div>
         ...
         <div className="rte-color__list">
            {colors.map((item) => (
               <ColorButton key={item} active={isColorEqual(item, color)} color={item} onClick={() => handleColorChange(item)} />
            ))}
         </div>
      </div>
   );
      ...
      <div className="rte-cp">
         <div className="rte-cp__tabs">
            {['swatches', 'custom'].map((tab) => (
               ...
               >
                  {tab === 'swatches' ? 'Paleta' : 'Personalizada'}
               </Button>
            ))}
         </div>
         ...
         <div className="rte-cp__main">
            {activeTab === 'swatches' && (
               <div className="rte-cp-swatches">
                  {renderColorList(COLORS, 'Cores padrao')}
                  {renderColorList(MORE_COLORS, 'Mais cores')}
               </div>
            )}

            {activeTab === 'custom' && (
               <div className="rte-cp-custom">
                  <HexColorPicker className="rte-cp-custom__picker" style={{ width: '100%' }} color={color} onChange={handleColorChange} />
                  <div className="rte-cp-custom__preview">
                     <ColorButton color={color} tooltip={false} />
                     <Input value={color!} style={{ textTransform: 'uppercase' }} onChange={(e) => handleColorChange(e.target.value)} autoFocus />
                  </div>
               </div>
            )}
         </div>
