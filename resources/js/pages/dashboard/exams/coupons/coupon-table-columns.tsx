...
const CouponTableColumns = ({ exams }: CouponTableColumnsProps): ColumnDef<ExamCoupon>[] => {
   const getCouponStatus = (coupon: ExamCoupon) => {
   ...
   return [
      ...
      },
      {
         ...
         cell: ({ row }) =>
            row.original.exam ? (
               <span className="font-medium">{row.original.exam.title}</span>
            ) : (
