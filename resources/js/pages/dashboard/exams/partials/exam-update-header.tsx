...
const ExamUpdateHeader = () => {
   ...
   return (
      <div className="flex flex-wrap items-center gap-4 md:gap-6">
         <Button>
            <Link href={route('exams.details', { slug: exam.slug, id: exam.id })}>Visualizar prova</Link>
         </Button>
         ...
         {user.role === 'instructor' && exam.status !== 'published' && (
            <Button onClick={() => router.put(route('exams.status', { exam: exam.id }), { status: 'published' })}>Enviar para revisão</Button>
         )}

         {user.role === 'admin' && (
            <Dialog open={open} onOpenChange={setOpen}>
               <DialogTrigger asChild>
                  <Button className="capitalize">Alterar status</Button>
               </DialogTrigger>
               <DialogContent>
                  <DialogHeader>
                     <DialogTitle>Alterar status da prova</DialogTitle>
                  </DialogHeader>
