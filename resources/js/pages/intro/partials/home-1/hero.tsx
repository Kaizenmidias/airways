...
const Hero = () => {
   ...
   const heroImage = heroSection?.thumbnail && heroSection.thumbnail !== '/assets/images/intro/home-1/hero-image.png' ? heroSection.thumbnail : '/assets/aviao.png';
   const heroAvatars = getPropertyArray(heroSection).slice(0, 4);
   const primaryButtonText = heroSection?.properties?.button_text || 'Explorar Cursos';
   const primaryButtonLink = heroSection?.properties?.button_link || '/courses/all';
   const secondaryButtonLink = route('category.courses', { category: 'all' });
         ...
         <div className="relative z-10 grid min-h-[calc(100vh-1.5rem)] grid-cols-1 lg:min-h-[calc(100vh-3rem)] lg:grid-cols-[1.08fr_0.92fr]">
            <div className="relative flex items-center overflow-hidden px-6 py-12 sm:px-10 sm:py-16 lg:px-12 lg:py-20">
               ...
               <div className="relative max-w-3xl text-white">
                  ...
                  <div className="mt-8 flex flex-wrap items-center gap-4">
                     <Button asChild className="h-12 rounded-full bg-[#FD122E] px-7 text-sm font-semibold text-white shadow-[0_18px_45px_rgba(253,18,46,0.35)] transition-transform hover:bg-[#e6112b] hover:scale-[1.01]">
                        <Link href={primaryButtonLink}>{primaryButtonText}</Link>
                     </Button>

                     <Button asChild variant="outline" className="h-12 rounded-full border-white/15 bg-white/10 px-7 text-sm font-semibold text-white shadow-none backdrop-blur-xl transition-colors hover:bg-white/15 hover:text-white">
                  ...
                  <div className="mt-10 flex flex-col gap-5 sm:flex-row sm:items-center sm:gap-8">
                     <div className="flex items-center gap-3 text-white/90">
                        ...
                        <p className="text-sm font-semibold">
                           {heroSection?.properties?.ratings || '5.0'}
            ...
            <div className="relative min-h-[420px] overflow-hidden bg-[#0f1b2f] lg:min-h-[780px]">
               <div className="absolute inset-0">
                  <img src={heroImage} alt={heroSection?.sub_title || heroSection?.title || 'Airways Academy'} className="h-full w-full object-cover object-center" />
