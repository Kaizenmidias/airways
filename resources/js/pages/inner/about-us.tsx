...
const AboutUs = () => {
   ...
   const identitySection = getPageSection(innerPage, 'success_statistics');
   const cardsSection = getPageSection(innerPage, 'mission_values_cards');

   const identityImage = identitySection?.thumbnail || '/assets/images/team-1.jpg';
   const identityBullets = (identitySection?.properties?.array as BulletItem[] | undefined)?.filter((item) => Boolean(normalizeArrayText(item)))?.length
      ? ((identitySection?.properties?.array as BulletItem[]) || fallbackBullets)
      : fallbackBullets;

   const cards = (cardsSection?.properties?.array as AboutCard[] | undefined)?.filter(Boolean)?.length
      ? ((cardsSection?.properties?.array as AboutCard[]) || fallbackCards)
      : fallbackCards;
   ...
   return (
      <div className="about-us-page bg-[#f4f7fb] text-slate-900">
                  ...
                  <div className="grid gap-10 lg:grid-cols-[minmax(0,1.08fr)_minmax(320px,0.92fr)] lg:items-center">
                     ...
                     <div className="max-w-[52rem] space-y-4 about-us-reveal lg:col-start-1 lg:row-start-2 lg:self-start">
                        ...
                        <div className="space-y-3 pt-2">
                           {identityBullets.map((bullet, index) => {
                              const bulletText = normalizeArrayText(bullet);

                              return (
                                 <div
                                    key={`${bulletText}-${index}`}
         ...
         <section className="bg-[#f4f7fb]">
            <Section customize={customize} pageSection={cardsSection} containerClass="!max-w-none !px-0" contentClass="relative">
               <div className="mx-auto max-w-[1600px] px-5 py-16 md:px-10 md:py-24 lg:px-14">
                  ...
                  <div className="mt-10 grid gap-6 lg:grid-cols-3">
                     {cards.map((card, index) => {
                        const bulletList = splitBullets(card.bullets);

                        return (
                           <article
                              key={`${card.title || 'card'}-${index}`}
                              className="group rounded-[28px] border border-slate-200 bg-white p-8 shadow-[0_12px_40px_rgba(15,23,42,0.05)] transition-transform duration-300 hover:-translate-y-1 about-us-reveal"
                              style={{ animationDelay: `${index * 120}ms` }}
                           >
